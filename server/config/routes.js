...
*/
  ...
  app.get('/imagecollection', images.all);

  app.post('/imagecollection', function(req, res) {
    images.add(req, res);
  });

  app.put('/imagecollection', function(req, res) {
    images.update(req, res);
  });

  // CHANGE THIS TO COLLECTIONS PAGE OR '/'

  app.get('*', function(req, res, next) {
    Image.find({}).exec(function(err, collections) {
      if(!err) {
        var imagemap = _.indexBy(collections, 'collectionid');
        var user = req.user ? { authenticated: true, isWaiting: false } : { authenticated: false, isWaiting: false };
        res.locals.data = {
          CollectionsStore: { allCollections: imagemap },
          UserStore: { user: user }
        };
        next();
      } else {
...
/*
  app.get('/collection/:collectionid', function(req, res, next) {
    ...
    Image.findOne({collectionid: collectionid}).exec(function(err, singlecollection) {
      if(!err) {
        res.locals.data = {
          ItemStore: {}
        };
